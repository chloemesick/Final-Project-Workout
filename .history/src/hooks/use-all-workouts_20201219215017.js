...
function useAllWorkouts(userId, filterDayOfWeek) {
  ...
  useEffect(() => {
    ...
    if (filterDayOfWeek === "All") {
      ...
      unsubscribe = workoutref.where("dayOf", "==", "Tuesday").onSnapshot(onNext, onError);
  ...
  unsubscribe = workoutref.where("Day", "==", "Friday").onSnapshot(onNext, onError);
  } else if (filterDayOfWeek === "Saturday") {
  unsubscribe = workoutref.where("Day", "==", "Saturday").onSnapshot(onNext, onError);
}
